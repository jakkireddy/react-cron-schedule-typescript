...
export const setRepeatCountAndTypeVal = (
  ...
  if (monthOption === MONTH_OPTIONS.STANDARD) {
    ...
    if (selectedMonthDayOrder === ORDERS.LAST) {
      const rType =
        selectedMonthDay === MONTH_DAY_TYPES.DAY
          ? REPEAT_TYPES.DAYS
          : selectedMonthDay === MONTH_DAY_TYPES.WEEKDAY
            ? REPEAT_TYPES.WORKING_DAYS
            : REPEAT_TYPES.WEEKS;
      repeatFor = 1;
      repeatForType = rType;
    } else {
      const rType = [MONTH_DAY_TYPES.DAY, MONTH_DAY_TYPES.WEEKDAY].includes(
        selectedMonthDay,
      )
        ? REPEAT_TYPES.DAYS
        : REPEAT_TYPES.WEEKS;
  ...
  if (
    ...
    ([
      ...
      MONTH_DAY_TYPES.SELECT_DAYS_MANUALLY,
...
export const getRepeatFromFreqType = (freqType) => {
  if ([REPEAT_OPTIONS.BI_WEEKLY].includes(freqType))
    return REPEAT_OPTIONS.WEEKLY;
  else if (
    [
