...
function MonthSelection(props) {
  ...
  const handleMonthOptionChange = (event) => {
    ...
    setValue({
      isFullWeek:
        val === MONTH_OPTIONS.CUSTOM &&
        [MONTH_DAY_TYPES.FULL_WEEK, MONTH_DAY_TYPES.FULL_WORKING_WEEK].includes(
          selectedMonthDay,
        ),
    });
    setRepeatCountAndTypeVal(
      setValue,
      setState,
      isRepeatForDisabled,
      val,
      ...
      <form className={css.container}>
        <div className={css.standardContainer}>
          ...
          <label
            style={{
              ...styles.dayLabel,
              color: `${monthOption !== MONTH_OPTIONS.STANDARD || disabled ? "gray" : ""}`,
            }}
        ...
        <div className={css.customContainer}>
          ...
          >
            {OrderOptions.map((item) => {
              const optionDisable =
                (![ORDERS.FIRST, ORDERS.LAST].includes(item) &&
          ...
          >
            {monthDayTypeOptions().map((item) => {
              const optionDisable =
                (item.value === MONTH_DAY_TYPES.WEEKDAY &&
                  ![ORDERS.FIRST, ORDERS.LAST].includes(
                    selectedMonthDayOrder,
                  )) ||
                ([
                  MONTH_DAY_TYPES.FULL_WEEK,
