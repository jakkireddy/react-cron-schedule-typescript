...
function RecurrenceComponent(props) {
  ...
  useEffect(() => {
    ...
    if (value?.cronExpression?.length > 0) {
      const schedule = getScheduleSettingsFromCronExp(
        value?.repeat,
      ...
      if (schedule) {
        data = {
          ...
          isAdditionalOptionsActive:
            value?.skipFrom !== undefined && value.skipTo !== undefined,
  ...
  useEffect(() => {
    ...
    if (state?.repeat === REPEAT_OPTIONS.WEEKLY) {
      ...
      state?.repeat === REPEAT_OPTIONS.MONTHLY ||
      state?.repeat === REPEAT_OPTIONS.YEARLY
    ) {
      if (state.monthOption === MONTH_OPTIONS.CUSTOM) {
        ...
        if (state.selectedMonthDay === MONTH_DAY_TYPES.DAY) {
          ...
          state.selectedMonthDay === MONTH_DAY_TYPES.FULL_WORKING_WEEK
        ) {
          if (state.selectedMonthDayOrder === ORDERS.LAST) {
            cronExp = [`0 0 ? * 5L`];
          } else {
          ...
          state.selectedMonthDay === MONTH_DAY_TYPES.SELECT_DAYS_MANUALLY
        ) {
    ...
    <div style={props?.styles?.root}>
      ...
      {state?.repeat === REPEAT_OPTIONS.WEEKLY ? (
        ...
        <>
          ...
          />
          {state.selectedMonthDay === MONTH_DAY_TYPES.SELECT_DAYS_MANUALLY &&
            state.monthOption === MONTH_OPTIONS.CUSTOM && (
              <WeekSelection
                {...props}
                setValue={setValue}
                state={state}
          ...
          {!(
            state.selectedMonthDayOrder === ORDERS.LAST &&
            state.monthOption === MONTH_OPTIONS.CUSTOM
          ) && (
            <RepeatFor
              {...props}
      ...
      {props.showCronExpression && (
        <div style={{ marginTop: 20 }}>{state?.cronExpression}</div>
      )}
    </div>
  );
}
