...
function RepeatFor(props) {
  ...
  const handleRepeatForTypeChange = (event) => {
    ...
    const count =
      val === REPEAT_TYPES.WEEKS && repeatForType !== REPEAT_TYPES.WEEKS
        ? 2
        : repeatFor;
    setValue({
      repeatForType: val,
  ...
  const handleRepeatForCheckbox = (event) => {
    ...
    const rType =
      ...
      [MONTH_DAY_TYPES.DAY, MONTH_DAY_TYPES.WEEKDAY].includes(
        selectedMonthDay,
      ) ||
      (selectedMonthDay === MONTH_DAY_TYPES.SELECT_DAYS_MANUALLY &&
        selectedWeeks?.length === 1 &&
        selectedMonthDayOrder === ORDERS.FOURTH)
        ? REPEAT_TYPES.DAYS
        : REPEAT_TYPES.WEEKS;
  ...
  const getRepeatForTypes = useCallback(() => {
    let arr = [
      { value: REPEAT_TYPES.DAYS, label: i18n.t("days") },
      { value: REPEAT_TYPES.WORKING_DAYS, label: i18n.t("working days") },
    ];
    if (
      monthOption === MONTH_OPTIONS.CUSTOM &&
      ![MONTH_DAY_TYPES.DAY, MONTH_DAY_TYPES.WEEKDAY].includes(selectedMonthDay)
    ) {
      arr = [{ value: REPEAT_TYPES.WEEKS, label: i18n.t("weeks") }];
      if (
        selectedWeeks?.length === 1 &&
        selectedMonthDay === MONTH_DAY_TYPES.SELECT_DAYS_MANUALLY
      ) {
        ...
        if (
          [ORDERS.FIRST, ORDERS.SECOND, ORDERS.THIRD].includes(
            selectedMonthDayOrder,
          )
        ) {
          arr.push({ value: REPEAT_TYPES.WEEKS, label: i18n.t("weeks") });
        }
      }
    }
    return arr;
    ...
    <div className={css.repeatForContainer}>
      <input
        disabled={
          disabled ||
          (monthOption === MONTH_OPTIONS.STANDARD &&
            Number(selectedMonthDate) > 27) ||
          (monthOption === MONTH_OPTIONS.CUSTOM &&
            selectedMonthDayOrder === ORDERS.FOURTH &&
            [
              MONTH_DAY_TYPES.FULL_WEEK,
              MONTH_DAY_TYPES.FULL_WORKING_WEEK,
            ].includes(selectedMonthDay))
        }
      ...
      <span
        style={{
          ...styles.forLabel,
          color: `${isRepeatForDisabled || disabled ? "gray" : ""}`,
        }}
